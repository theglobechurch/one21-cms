    ...
    function validateType(f, validTypes) {
      return new Promise((resolve, reject) => {
        if (!f.type.match(`(?:${validTypes})`)) {
          reject(
            `File uploaded must be one of [${validTypes.replace(
              "|",
              ", "
            )}] but was ${f.type}`
          );
        ...
        preview.onload = function(pr) {
          ...
          dimension_test.onload = function() {
            if (this.width < 1024 || this.height < 768) {
              reject(
                `Image must be bigger than 1024x768px, but instead was ${
                  this.width
                }x${this.height}px.`
              );
            } else {
      ...
      <div className="imageUpload">
        ...
        <div className="form__field">
          ...
          >
            ...
            <div className="imageUpload__btnGroup">
              ...
              <label htmlFor="dropzone-file-field" className="btn">
                Upload
              </label>
            </div>
        ...
        {this.state.errors.length > 0 && <Toast messages={this.state.errors} />}
      </div>
    );
  }
}
